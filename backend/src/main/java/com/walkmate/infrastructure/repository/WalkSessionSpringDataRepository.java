...
public interface WalkSessionSpringDataRepository extends JpaRepository<WalkSessionJpaEntity, UUID> {
    ...
    @Query(value = """
            ...
            AND scheduled_start_time < :scheduledEndTime
            AND scheduled_end_time > :scheduledStartTime
            """, nativeQuery = true)
    boolean hasOverlappingPendingOrActive(
            @Param("userId") UUID userId,
            @Param("scheduledStartTime") Instant scheduledStartTime,
            @Param("scheduledEndTime") Instant scheduledEndTime);
}
