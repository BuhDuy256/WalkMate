...
public class JpaSessionRepository implements SessionRepository {
    ...
    @Override
    public boolean hasOverlappingPendingOrActive(UUID userId, Instant scheduledStartTime, Instant scheduledEndTime) {
        return walkSessionRepository.hasOverlappingPendingOrActive(userId, scheduledStartTime, scheduledEndTime);
    }
    ...
    private WalkSessionJpaEntity toJpa(WalkSession session) {
        ...
        entity
                .setAbortReason(
                        session.getAbortReason() == null ? null : AbortReason.valueOf(session.getAbortReason().name()));
